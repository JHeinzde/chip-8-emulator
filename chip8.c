...
void load_program(char *filename, struct chip8 *chip) {
    ...
    size_t result = fread(chip->memory+0x200, 1, MEM_SIZE-0x200, file);
...
void emulate_cycle(struct chip8 *chip) {
    ...
    switch (chip->opcode & BIT_16) {
        ...
        case DRW: {
            ...
            uint8_t height = n; // The 'n' or nibble from the instruction
            chip->V[VF] = 0; // Reset collision flag
        ...
        case OPS_M:
            switch (nn) {
                ...
                case LD_K_W: {
                    ...
                    if (!key) {
                        chip->pc -= 2; // repeat this instruction until key was pressed
                    } else {
...
void draw_graphics(struct chip8 *chip, RenderTexture2D target) {
    ...
    ClearBackground(BLACK); // Clear the internal 64x32 canvas
...
void execute_program(struct chip8 *chip) {
    ...
    while (!WindowShouldClose()) {
        ...
        set_keys(chip);
        // We need to draw the texture to the screen every time.
        BeginDrawing();
        ClearBackground(BLACK);

        // Scale the 64x32 texture to the full window size
...
unsigned char key_code_to_key_pad(int key_code) {
    switch (key_code) {
        ...
        default:
            return INVALID_KEY; // Not a Chip-8 key
    }
}